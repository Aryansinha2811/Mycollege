...
int main() 
{ // Starting point of the program, where execution begins
   ...
   for(int i=0; i<3; i++) 
   { // Starting an outer loop that iterates from i=0 to i<3 with i incrementing by 1 each iteration
      for(int j=0; j<3; j++) 
      { // Starting an inner loop that iterates from j=0 to j<3 with j incrementing by 1 each iteration
