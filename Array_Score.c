...
{ // Starting point of the program, where execution begins
   ...
   for(int i=1; i<5; i++) 
   { // Starting a loop that iterates from i=1 to i<5 with i incrementing by 1 each iteration
      if(secured[i] > max_score) 
      { // Checking if the element at index 'i' in the 'secured' array is greater than 'max_score'
